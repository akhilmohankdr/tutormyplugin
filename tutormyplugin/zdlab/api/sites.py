...
from tutormyplugin.zdlab.api.helpers import as_course_key


def get_course_keys_for_site(site):
    orgs = Organization.objects.filter(name__in=[site])
    org_courses = OrganizationCourse.objects.all()
...
# get_course_keys_for_site(site='school1')